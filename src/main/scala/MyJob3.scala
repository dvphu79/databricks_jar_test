...
object MyJob3 {
  def main(args: Array[String]): Unit = {
    ...
    println("OAuth 2.0 with an Azure service principal")
    ...
    configConnectionToSynapse()
    val tableName = "People"
    val directoryName = "data"
    uploadTransformedDataToSynapseTable(transformedDataFrame1, tableName, containerName, storageAccountName, directoryName, "overwrite")
    uploadTransformedDataToSynapseTable(transformedDataFrame2, tableName, containerName, storageAccountName, directoryName, "append")
    uploadTransformedDataToSynapseTable(transformedDataFrame3, tableName, containerName, storageAccountName, directoryName, "append")
    uploadTransformedDataToSynapseTable(transformedDataFrame4, tableName, containerName, storageAccountName, directoryName, "append")
    uploadTransformedDataToSynapseTable(transformedDataFrame5, tableName, containerName, storageAccountName, directoryName, "append")
  ...
  private def configConnectionToSynapse(): Unit = {
    val serviceCredential = dbutils.secrets.get(scope = "key-vault-secret-1", key = "synapse-databricks-secret")
    val storageAccountAccessKey = dbutils.secrets.get(scope = "key-vault-secret-1", key = "storage-account-access-key")
  ...
  private def uploadTransformedDataToSynapseTable(df: DataFrame, tableName: String, containerName: String, storageAccountName: String, directoryName: String, mode: String): Unit = {
    val username = "sqladminuser@synapse79"
    val password = dbutils.secrets.get(scope = "key-vault-secret-1", key = "sql-admin-password")
